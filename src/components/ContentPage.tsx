          ...
          {page.sections.map((section, sectionIndex) => (
            <section key={`${section.title ?? "section"}-${sectionIndex}`}>
              ...
              {section.items && <div className="mt-6 grid gap-3 sm:grid-cols-2">{section.items.map((item) => {
                const content = <>{item.image && <div className="relative -mx-5 -mt-5 mb-4 aspect-[16/9] overflow-hidden rounded-t-xl"><Image src={item.image} alt="" fill sizes="(max-width: 640px) 100vw, 50vw" className="object-cover" /></div>}<h3 className="font-display text-lg font-semibold text-pine-dark">{item.title}</h3>{item.detail && <p className="mt-1 text-sm leading-relaxed text-ink-soft">{item.detail}</p>}</>;
