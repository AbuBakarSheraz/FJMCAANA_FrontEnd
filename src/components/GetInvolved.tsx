  ...
  return (
    <section id="involved" className="max-w-6xl mx-auto px-6 py-20">
      ...
      <div className="grid sm:grid-cols-3 gap-5">
        {CARDS.map((card) => (
          <div key={card.title} className="border border-burgundy/10 rounded-xl p-6 bg-white">
            <h3 className="font-display font-semibold text-lg text-burgundy-dark">{card.title}</h3>
            <p className="text-sm text-ink-soft mt-2">{card.body}</p>
            <a href={card.href} className="inline-block mt-4 text-sm font-semibold text-burgundy">
              {card.cta} →
            </a>
          </div>
        ))}
