...
function mapCloudinaryResource(
  ...
  return {
    ...
    thumb: {
      avif: transform(
        "w_600,c_limit,q_auto,f_avif"
      ),

      webp: transform(
        "w_600,c_limit,q_auto,f_webp"
      ),

      fallback: transform(
        "w_600,c_limit,q_auto,f_jpg"
      ),
    },
    ...
    blur: transform(
      "w_40,c_limit,q_10,f_jpg,e_blur:800"
    ),
...
async function loadCloudinaryGallery(
   ...
   * IMPORTANT:
   *
   * The Cloudinary asset folder can contain
   * both images and videos.
   *
   * This API must return ONLY images.
   */
...
export async function GET(
  ...
  try {
     ...
     * use the existing local gallery.
     */
    if (
      !hasCloudinaryConfig()
    ) {
     ...
     */
    console.warn(
      `[gallery] No Cloudinary images found in asset folder "${folder}"`
    );
